...
def get_prediction(input_arr, model_key="RISE"):
    logger.debug("Number of data points obtained is {}".format(len(input_arr)))
    input_arr = ret_splitted_array(input_arr)
    logger.debug("Number of data points obtained after truncation {}".format(len(input_arr)))
    if len(input_arr) == 0:
        return 'None'
    if model_key not in models_dict:
        logger.debug("No such model exists")
        return None
